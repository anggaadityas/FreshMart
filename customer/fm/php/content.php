    ...
    if ($jumlah%$limit) { 
      ...
      for ($i=1;$i<=$halaman;$i++) {
        $newoffset=$limit*($i-1); 
          if ($offset!=$newoffset)
            { 
            echo "<li><a href=\"home-$newoffset.html\">$i</a></li>";
          } else { 
            echo "<li class=\"active\"><a href=\"home-$newoffset.html\">$i</a></li>"; 
          } 
      }
            echo "</div></ul>";
...
<?php
   $sql = mysql_query("SELECT * FROM info ORDER BY id_info DESC LIMIT 0,3");
  if(mysql_num_rows($sql) == 0)
  {
    echo "Tidak ada info produk!";
  }
  else
  {
